...
corkboard, review them in an outline, and compile the tree into a
submission manuscript, a typeset book, or a screenplay.

Every document is a plain `.md` file, or a `.fountain` file for
screenplays. A small JSON file next to each document holds its
metadata: label, status, synopsis, keywords, and word-count target.
The tree itself is one `project.json` per scrive. You can edit all of
these files with any editor.

## Requirements

- **Vim 9.2 or newer**, with `+popupwin`.[^vim]
- **[Logger](https://github.com/Galvarcus/Logger)** - a Vim9script
...
[^vim]: The binder tree uses a Vim9 class with a field that is a list
    ...
    Run `vim --version` to check. Bartleby does not support Vim 8 or
    Neovim.

## Installation

Install Logger and Bartleby the same way.
...
Bartleby asks for the scrive type: novel, novel with parts, short
story, or screenplay. It creates the scrive under
`g:bartleby_binder_root` and opens the Binder with a starter tree:
Front Matter, Manuscript with a first chapter and scene, Back Matter,
Characters, and Research.

In the Binder:

- `<CR>` on a document opens it.
- `a` adds a document. `A` adds a folder.
- `go` opens the Outliner for the folder under the cursor. `gc` opens
  its Corkboard.

Elsewhere:

- `<leader>bi` toggles the Inspector for the current document.
- `:BartlebyCompile` compiles the scrive.
- `:BartlebyOpen` reopens the last scrive. `:BartlebyList` lists all
  scrives.

## Concepts

**Scrive** - one writing project. It is a folder under
`g:bartleby_binder_root` that holds `project.json`, a `binder/`
folder with the documents, and a `compile/` folder with compile
targets and their output.
...
The Binder is the sidebar with the scrive's tree. It opens with the
scrive, and `:BartlebyToggleBinder` toggles it. Line 1 shows the
scrive title. Folders end with `/`. A document with a label shows the
label color after its title. Long titles wrap.
...
`go` on a folder shows all items under it as a table: Title, Label,
Status, Words, Target, and Keywords. Rows are indented like the tree.
...
`:BartlebyList` shows every scrive with its title and type. If no
scrive exists, it asks for a name and creates one. `:BartlebyOpen`
with no name and no previous scrive also shows this list.
...
Three independent writing aids:
...
filetype and syntax highlighting. Spotlight's Dialogue mode finds
character cues and scene headings in them.

## Palette and menu

`:BartlebyCommands` or `<leader>b<Space>` opens a searchable list of
all Bartleby commands. `:BartlebyMenu` or `<leader>bm` shows the same
commands in groups: Scrive, Binder, View, Document, and Project.

## Session, snapshots, and search

**Session** - Bartleby remembers, for each scrive, the open document,
the cursor position, and which Binder folders are collapsed. It also
remembers the last scrive. To reopen it when Vim starts, set
`g:bartleby_session_auto_restore`.

**Snapshots** - `S` in the Binder or `:BartlebySnapshot` saves a
timestamped copy of a document. `gS` or `:BartlebySnapshots` lists
the copies and restores one. A restore first takes a snapshot of the
current text.
...
window and in Focus mode. Other files are not affected.

- When you pause in Normal mode or leave Insert mode, it saves at most
  once every `g:bartleby_autosave_interval` seconds. A save that the
  interval skips runs when the interval ends.
- When you leave the document or Vim loses focus, it saves at once.

It writes only changed documents. To turn it off, set
`g:bartleby_autosave` to `0`.

## Dictionary and thesaurus

These need Merriam-Webster API keys. See [Configuration](#configuration).

`<leader>bd` shows the definition of the word under the cursor.
`<leader>bt` shows its synonyms. In Visual mode, both look up the
selected text on one line. `:BartlebyDefine {word}` and
`:BartlebyThesaurus {word}` look up a typed word.

| Key | Action |
| --- | --- |
| `j` / `k` | Move the selection |
| `<CR>` in the thesaurus | Replace the word in the text, with the same capitalization. For a typed word, copy the synonym to the unnamed register |
...
One `u` undoes a replacement. A misspelled word shows Merriam-Webster's
suggestions.

Results are cached in `~/.bartleby/lexicon_cache.json`, so a repeated
lookup sends no request. The cache does not contain your keys.

Each lookup needs its own key. Without the key, its mapping and its
palette and menu entries are not created, and its command reports the
missing setting. Set the keys before Bartleby loads, or use the
`$BARTLEBY_MW_DICTIONARY_KEY` and `$BARTLEBY_MW_THESAURUS_KEY`
environment variables to keep them out of a shared vimrc.

## Compile

`:BartlebyCompile` asks for the documents to include, a kind, a
format, and settings for that kind such as font, line spacing, and
cover image. It saves the answers as a named target that you can run
again.

The selection pane lists the contents of Front Matter, Manuscript,
and Back Matter. Only these folders go into a compile.

| Kind | Formats | Result |
| --- | --- | --- |
| Manuscript | PDF | Submission format through sffms: double-spaced Courier, running header, and a title page with the word count |
| Book | PDF, HTML, EPUB, Markdown | A typeset book with chapter openings, running headers, a table of contents, and an optional cover image |
| Screenplay | PDF, HTML, FDX | Standard screenplay format through screenplain |

The tree sets the structure. A Chapter folder becomes a numbered
chapter, and each Front Matter and Back Matter document becomes an
...
| --- | --- |
| `x` | Include or exclude the document. On a folder, all documents under it |
...
| `:BartlebySpotlight[!] [mode or coefficient]` | Toggle Spotlight. `!` turns it off |
| `:BartlebyQuill [detect, off, hard, soft, or toggle]` | Set the Quill wrap mode |
| `:BartlebySnapshot` | Take a snapshot of the current document |
| `:BartlebySnapshots` | View or restore snapshots of the current document |
...
The Binder, Corkboard, Outliner, and other panes have their own keys,
listed in their sections.

## Configuration

Set these variables in your vimrc before Bartleby loads. Each has a
default, so set only the ones you want to change.
...
| Variable | Default | Description |
| --- | --- | --- |
| `g:bartleby_focus_width` | `80` | Width of the writing column |
| `g:bartleby_focus_height` | `'85%'` | Height of the writing column |
| `g:bartleby_focus_margin_top` | `-1` | Top margin. `-1` centers the column |
...
| Variable | Default | Description |
| --- | --- | --- |
| `g:bartleby_quill_wrap_mode_default` | `'hard'` | Wrap mode when detection finds no clear style |
| `g:bartleby_quill_textwidth` | `74` | `textwidth` in hard-wrap mode |
| `g:bartleby_quill_autoformat` | `1` | Reformat paragraphs as you type |
| `g:bartleby_quill_autoformat_config` | Per filetype | `formatoptions` changes for each filetype |
| `g:bartleby_quill_autoformat_aliases` | A filetype map | Filetypes that share one autoformat setting |
| `g:bartleby_quill_joinspaces` | `0` | Two spaces after a sentence when lines are joined |
| `g:bartleby_quill_cursorwrap` | `1` | Move the cursor by screen line in soft-wrap mode |
| `g:bartleby_quill_conceallevel` | `3` | `conceallevel` in soft-wrap mode |
| `g:bartleby_quill_concealcursor` | `'c'` | `concealcursor` in soft-wrap mode |
| `g:bartleby_quill_soft_detect_sample` | `20` | Lines read to detect the wrap style |
| `g:bartleby_quill_soft_detect_threshold` | `130` | Sensitivity of the detection |
| `g:bartleby_quill_mode_indicators` | Statusline text | Text shown for each wrap mode |
| `g:bartleby_quill_auto` | `1` | Start Quill for scrive documents automatically |

**Compile**

| Variable | Default | Description |
| --- | --- | --- |
| `g:bartleby_compile_pandoc_bin` | `'pandoc'` | Pandoc program |
| `g:bartleby_compile_screenplain_bin` | `'screenplain'` | screenplain program |
| `g:bartleby_compile_toc` | `0` | Add a table of contents |
| `g:bartleby_compile_standalone` | `1` | Pass `--standalone` to Pandoc |
| `g:bartleby_compile_manuscript_font` | `'Courier New'` | Manuscript font. sffms uses Courier or not Courier |
| `g:bartleby_compile_book_font` | `'Georgia'` | Book font |
| `g:bartleby_compile_screenplay_font` | `'Courier Prime'` | Screenplay font |
| `g:bartleby_compile_manuscript_double_spaced` | `1` | Line spacing of new Manuscript targets |
| `g:bartleby_compile_indent_paragraphs` | `1` | Indent Book paragraphs instead of spacing them |
